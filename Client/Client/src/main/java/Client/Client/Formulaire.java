...
public class Formulaire {
	...
	private void createView() {
		...
		depot.setPreferredSize(new Dimension(1300, 1000));
		// Titre de la stb
		...
		model1.addRow(new Object[]{"Leneveu", "Lucie", "false"});
		model1.addRow(new Object[]{"Simeon", "Guillaume", "true"});
		...
		addRow1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (model1.getRowCount() < 7) {
					...
					model1.addRow(new Object[]{"Nom", "Prenom", "76000"});
		...
		valider.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (title.equals("")
					...
					JOptionPane.showMessageDialog(null,  "La stb n'est pas au bon format", "Ajout Stb", 1);
				}
				...
				for (int i = 0; i < model1.getRowCount(); ++i) {
					...
					if(!model1.getValueAt(i, 2).toString().equals("true") 
						&& !model1.getValueAt(i, 2).toString().equals("false")) {
						membre = false;
					}
					// On complète notre chaîne
					xml += "\t <p:Equipe> \n"
							+ "\t\t <p:gender>" + model1.getValueAt(i, 2) + "</p:gender> \n"
